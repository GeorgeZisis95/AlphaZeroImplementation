...
tensor_state = torch.tensor(encoded_state, device=device).unsqueeze(0) # Makes it a tensor and adds extra dimension
print(tensor_state)
...
value = value.item() # Gets the tensor value
policy = torch.softmax(policy, axis=1).squeeze(0).detach().cpu().numpy() # Performs softmax to the correct axis
                                                                         # Removes one dimension, removes gradient
                                                                         # Sends to cpu and makes it nparray
